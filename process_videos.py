...
def videos_to_frames(input_path, output_path, interval, dict_directory):
    ...
    for video_name in listing:
        ...
        for i in range(1, frames-1, interval):
            ...
            if dict_directory:
                cv.imwrite(output_path+str(i)+"_rgb_.png", frame)
                cv.imwrite(output_path+str(i)+"_flow_.png", flow_frame)
            else:
                if not os.path.exists(output_path+video_name[:-4]):
                    os.mkdir(output_path + video_name[:-4])
                cv.imwrite(output_path+video_name[:-4]+"/"+str(i)+"_rgb_.png", frame)
                cv.imwrite(output_path+video_name[:-4]+"/"+str(i)+"_flow.png", flow_frame)


if __name__ == '__main__':
    videos_to_frames(r"/home/user/input_path/", r"/home/user/output_path/", 20, False)
